...
def write_debug_report(outputs: dict[str, pd.DataFrame], quick: bool) -> Path:
    ...
    report_path = dirs["ablation"] / "deep_debug_report.md"
    overfit = outputs["overfit"]
    cv = outputs["branch_ablation_cv"]
    locked = outputs["branch_ablation_locked"]
    evaluated_cv = cv[cv["status"] == "evaluated"] if "status" in cv else cv
    cv_summary = (
        evaluated_cv.groupby(["dataset", "scenario", "variant"], as_index=False)
    ...
    lines = [
        ...
        "## Overfit Sanity",
        "",
        _markdown_table(overfit, ["dataset", "scenario", "variant", "status", "macro_f1", "recall_low", "f1_low", "rmse", "r2", "regression_head_rmse"]),
        "",
        "## Branch Ablation CV",
        "",
        _markdown_table(cv_summary, ["dataset", "scenario", "variant", "macro_f1", "recall_low", "f1_low", "rmse", "r2"]),
        "",
        "## Branch Ablation Locked Test",
        "",
        _markdown_table(locked.sort_values("macro_f1", ascending=False) if "macro_f1" in locked else locked, ["dataset", "scenario", "variant", "status", "macro_f1", "recall_low", "f1_low", "rmse", "r2", "regression_head_rmse"]),
    ]
