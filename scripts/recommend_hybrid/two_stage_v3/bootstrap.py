"""Learner-cluster bootstrap for Two-Stage V3 held-out predictions."""
...
def main() -> None:
    ...
    if missing:
        raise RuntimeError(f"OOF predictions missing columns: {sorted(missing)}")
    ...
    if len(learners) < 2:
        raise RuntimeError("learner bootstrap requires at least two clusters")
    ...
    point_gate_recall = point_coverage
    ...
    gate_recall = np.zeros(REPLICATES, dtype=np.float64)
    ...
    for replicate in range(REPLICATES):
        ...
        gate_recall[replicate] = coverage[replicate]
        conditional[replicate] = totals[1] / totals[3] if totals[3] else 0.0
    result = {
        "schema_version": "two_stage_v3_learner_bootstrap_v1",
        "status": "COMPLETE",
        ...
        "stage_a_recall": _summary(gate_recall, point_gate_recall),
        "stage_b_conditional_precision_at_1": _summary(
            conditional, point_conditional
        ),
