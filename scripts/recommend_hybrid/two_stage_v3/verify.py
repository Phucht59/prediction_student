"""Verify Two-Stage V3 authority, leakage boundaries, and exact replay."""
...
def _head_config(payload: dict) -> TwoStageHeadConfig:
...
def main() -> None:
    ...
    registry = json.loads(
        (OUT / "cache/CACHE_REGISTRY.json").read_text(encoding="utf-8")
    )
    results = json.loads(
    ...
    action_candidates = pd.read_parquet(OUT / "cache/ACTION_CANDIDATES.parquet")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    replay_rows = []
    checkpoint_authority = []

    for outer_fold in protocol["evaluation"]["outer_folds"]:
        selected_path = OUT / f"final_oof/fold_{outer_fold}/selected.json"
        selected = json.loads(selected_path.read_text(encoding="utf-8"))
        group_features = pd.read_parquet(
            OUT / f"cache/outer_{outer_fold}/GROUP_FEATURES.parquet"
        )
        arrays, schema = load_two_stage_arrays(group_features, action_candidates)
        if schema != selected["feature_schema"]:
            raise RuntimeError(f"fold {outer_fold}: feature schema drift")
        ...
        for checkpoint in selected["checkpoints"]:
            ...
            if _sha256(checkpoint_path) != checkpoint["sha256"]:
                raise RuntimeError("head checkpoint checksum mismatch")
            payload = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
            if payload.get("frozen_backbone_trainable") is not False:
                raise RuntimeError("head checkpoint authorizes backbone training")
            state_dict = payload["state_dict"]
            if any("backbone" in name or "tabular_expert" in name for name in state_dict):
                raise RuntimeError("head checkpoint contains prediction-backbone parameters")
        ...
        gate_logits = np.mean(gate_rows, axis=0)
        ...
        thresholds = TwoStageThresholds(
            ...
            minimum_action_margin=float(
                selected["thresholds"]["minimum_action_margin"]
            ),
    ...
    exact_ids = np.array_equal(official["group_id"].to_numpy(), replay["group_id"].to_numpy())
    numeric_columns = [
        "gate_logit",
    ...
    discrete_match = np.array_equal(
        official[discrete_columns].to_numpy(), replay[discrete_columns].to_numpy()
    )
    ...
    training_script = ROOT / "scripts/recommend_hybrid/two_stage_v3/train_and_evaluate.py"
    ...
    verification = {
        "schema_version": "two_stage_v3_verification_v1",
