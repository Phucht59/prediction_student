...
## Đọc repo khi vấn đáp

| Cần xem | Vị trí chính | Vai trò |
|---|---|---|
| Code mô hình | `src/models/` | Kiến trúc CNN, BiLSTM, CNN–BiLSTM và OULAD Tabular Residual |
| Code pipeline | `src/data/`, `src/pipelines/`, `src/training/`, `src/evaluation/` | Dữ liệu, tiền xử lý, huấn luyện và đánh giá |
| Cấu hình final | `configs/final/` | Model/config authority đã đóng băng |
| Code recommendation | `src/recommend_hybrid/final/` | Pipeline, policy, safety router và LearningPlan |
| Code database | `src/database/`, `database/final/` | Runtime PostgreSQL và schema/migration cuối |
| Bằng chứng máy đọc | `artifacts/final/`, `artifacts/canonical_v3/` | Checkpoint, prediction, metric và checksum |
| Kết quả để trình bày | `reports/final/thesis_v3/` | Bộ bảng kết quả hiện hành dùng cho khóa luận |
| Điểm vào | `project.py`, `scripts/final/`, `scripts/recommend_hybrid/final/` | CLI và các bước kiểm chứng release |

Quy ước ngắn: **code nằm trong `src/`; cấu hình authority nằm trong `configs/final/`; bằng chứng số nằm trong `artifacts/`; báo cáo đọc trực tiếp nằm trong `reports/final/thesis_v3/`**. Các thư mục phase/audit và `experiments/` chỉ phục vụ provenance, không phải nơi bắt đầu đọc implementation cuối.
...
### 7.1. Chính sách thực tế của các mô hình final

Mô hình final **không dùng một chiến lược cân bằng lớp duy nhất cho cả ba dataset**. Audit authority xác nhận:

| Dataset | Chính sách imbalance của authority | Uniform NONE? | Macro-F1 authority |
|---|---|---|---:|
| Student-Mat | `MIXED_EFFECTIVE_LOSS` | Không | **0.901460** |
| Student-Por | `MIXED_FOLD_SPECIFIC` | Không | **0.862259** |
| OULAD | `UNIFORM_NONE_STANDARD_BCE` | Có | **0.894071** |

Vì vậy không mô tả toàn bộ hệ thống là “không xử lý mất cân bằng”. Student-Mat có weighting/effective-loss trong policy lịch sử; Student-Por dùng policy theo fold và có fold sử dụng class weighting; riêng OULAD final dùng Standard BCE, không tái cân bằng lớp.

SMOTE và ADASYN **không nằm trong authority final** của ba dataset. Các kỹ thuật resampling chỉ được dùng trong thí nghiệm sensitivity/diagnostic và luôn giới hạn ở training partition để tránh data leakage.

### 7.2. Frozen-embedding sensitivity evidence

Đề tài đã đóng băng biểu diễn Hybrid 96 chiều và huấn luyện cùng một Logistic Regression head theo bốn chế độ `none`, `class_weight`, `SMOTE`, `ADASYN`. Đây là sensitivity evidence, không phải phép so sánh trực tiếp giữa các checkpoint Hybrid final.
...
Trong sensitivity study này, `none` có F1, recall, balanced accuracy và Brier tốt nhất; synthetic oversampling tăng precision/specificity nhưng giảm recall. Kết quả này chỉ cho thấy resampling không tự động cải thiện representation đã đóng băng.

### 7.3. Authority-equivalent verification

Sau audit protocol, các control được phục hồi theo đúng authority:

- Student-Mat: reproduction Macro-F1 `0.9014601961315334`, khớp authority `0.9014601961315334`.
- Student-Por: fixed-NONE đạt `0.8614773022644222`, nhưng đây **không phải authority** vì authority dùng policy hỗn hợp theo fold. Authority-policy replay đạt đúng `0.8622587167738002`.
- OULAD: authority thực tế là `UNIFORM_NONE_STANDARD_BCE`. Protocol-equivalent NONE reproduction đạt `0.8942454181505014`, so với frozen authority `0.8940709888551659`; chênh lệch nhỏ được forensic audit quy cho CUDA nondeterminism của môi trường lịch sử, trong khi record, fold, label, threshold policy và five-seed aggregation đều khớp.

### 7.4. OULAD controlled Class Weight challenge

Vì OULAD final dùng Standard BCE, nhóm chạy thêm thí nghiệm authority-equivalent `FIXED_CLASS_WEIGHT` trên đúng kiến trúc H1 Tabular Residual CNN–BiLSTM, 3 outer folds × 5 seeds = 15 jobs. Kiến trúc và protocol giữ nguyên; class weighting chỉ thay đổi loss policy và được tính từ training partition.
...
Class Weight giúp Risk Recall tăng từ `0.784267` lên `0.826008`, nghĩa là phát hiện thêm sinh viên nguy cơ. Tuy nhiên Macro-F1 giảm `0.008262` và PR-AUC gần như không đổi. Với Macro-F1 là chỉ số chính của benchmark, trade-off này không đủ để thay final authority.

Do đó:

- `CLASS_WEIGHT_WINS=FALSE`.
- Không tạo OULAD Final V2.
- Kiến trúc H1 vẫn giữ nguyên **160,492 tham số**.
- OULAD final tiếp tục dùng **Standard BCE / không tái cân bằng lớp**.
- Frozen headline Macro-F1 vẫn là **0.894071**; không thay bằng metric của reproduction experiment.

### 7.5. Kết luận về mất cân bằng

> **Xử lý mất cân bằng không mang lại lợi ích nhất quán trên cả ba bộ dữ liệu. Policy final được giữ theo từng dataset thay vì ép một kỹ thuật chung. Riêng OULAD, Class Weight tăng Risk Recall nhưng làm Macro-F1 giảm rõ rệt, vì vậy Standard BCE được giữ làm authority final.**
...
- `reports/final/imbalance_evidence/OULAD_CLASS_WEIGHT_CHALLENGE.md`
...
- Synthetic imbalance sampling, khi được dùng trong sensitivity study, chỉ được fit trên training partition.
- Class weights trong controlled challenge chỉ được tính từ training partition; validation và outer test không tham gia tính trọng số.
...
- Class Weight trên OULAD cải thiện Risk Recall nhưng làm giảm Macro-F1, thể hiện trade-off giữa độ nhạy lớp nguy cơ và hiệu năng cân bằng tổng thể.
...
The final prediction authority remains the frozen H1 residual CNN–BiLSTM
(160,492 parameters). Recommendation V2 consumes its frozen risk outputs; it
does not retrain, replace, or tune the prediction model. Five action-specific
EBMs score feasible actions on an ordinal 0–3 scale, and the public adapter
normalizes only once with `clip(native_prediction / 3, 0, 1)`.

The released recommendation scope is the one-shot Panel-B held-out evaluation
(150 cases and 557 real external review records). It is authorized for runtime
integration only under the exact hashes in the final release manifest. No
post-Panel-B tuning and no causal treatment-effect claim are permitted.

Current authority and report reading order are recorded in
`reports/CURRENT_REPORTS.md`. The frozen release manifest is
...
| OULAD Class Weight challenge | `reports/final/imbalance_evidence/OULAD_CLASS_WEIGHT_CHALLENGE.md` |
...
- Bằng chứng imbalance cho thấy không có một kỹ thuật cân bằng duy nhất phù hợp cho mọi dataset; riêng OULAD, Class Weight tăng Risk Recall nhưng giảm Macro-F1 nên Standard BCE vẫn được giữ làm authority final.
