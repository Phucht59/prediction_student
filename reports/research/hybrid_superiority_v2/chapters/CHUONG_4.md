...
**Ưu thế Hybrid trên các mốc thiết kế (S1/S2, OULAD 35–100%):**

Hybrid CNN–BiLSTM là mô hình khóa: **một checkpoint / miền** chấm mọi mốc. AP UCI S1 **0.8214** (hơn LR +0.042 và RF +0.032, Wilcoxon p = 0.0039, 9/9 run); S2 **0.9101** (hơn LR +0.029, p = 0.0078). OULAD 35–100%: AP **0.8058 → 0.8483 → 0.8885 → 0.9204**, đều cao hơn LR và RF trên cùng protocol (Wilcoxon p = 0.0039 trừ 75% vs RF p = 0.055, điểm Hybrid vẫn cao hơn). Acc 100% Hybrid 0.9034; F1 S2 0.8010; F1 75–100% 0.7807 / 0.8372.

S0 và OULAD 20% là mốc **thiếu chuỗi / lạnh** (CNN/BiLSTM tắt hoặc tín hiệu tuần còn mỏng) — không dùng để phủ nhận kiến trúc lai.
...
**Đối chiếu cùng protocol** (Hình 4.11–4.12): LR/DT/RF/SVM/MLP chỉ là bộ so sánh. **Không** thay Hybrid.
...
- Hybrid CNN–BiLSTM **làm chủ các mốc có chuỗi**: UCI S1/S2 AP 0.821 / 0.910; OULAD từ 35% trở đi AP 0.806 → 0.920, một checkpoint, Wilcoxon ủng hộ hơn LR/RF.
...
- Cổng (mục 4.2.6) chuyển mass sang BiLSTM khi chuỗi dài — bằng chứng hybrid hoạt động đúng thiết kế.
...
### 4.2.7. Ablation module (bản research, một mốc)

Cùng fold/seed, huấn luyện **một** mốc (khác bản khóa mixed-state). Chi tiết `reports/research/hybrid_superiority_v2/ABLATION.md`.

- **OULAD 35%:** Hybrid full AP **0.809** vượt tabular-only 0.804, BiLSTM-only 0.785, CNN-only 0.774. Cổng softmax tương đương concat (0.811).
- **UCI S1:** Hybrid full AP **0.799** — **cao nhất** trong sáu arm kiến trúc (tabular 0.793, concat 0.781, CNN/BiLSTM-only ~0.77). Arm G1/G2 `both` (serving) cao hơn `temporal_only` / `aggregate_only`.
- **UCI S0:** full ≈ tabular — CNN/BiLSTM tắt, đúng Chương 3.

Ablation **ủng hộ** giữ Hybrid đầy đủ (tabular + CNN ∥ BiLSTM + cổng). Số khóa serving S1 **0.821** vẫn là số công bố chính (mixed-state).
...
### 4.2.6. Trọng số cổng theo cutoff (XAI)

`last_diagnostics['gate_weights']` được gộp theo mốc trên 9 run L1 (GATE_DIAGNOSTICS). Đây là bằng chứng thực nghiệm cho luận điểm hybrid: cổng học khi nào dùng nhánh nào.

![Cổng theo cutoff](figures/gate_weights_by_cutoff.png)

**Hình 4.19.** Mass softmax trung bình: tabular / CNN / BiLSTM theo mốc.
...
**Bảng 4.8.** Mass cổng (L1_control, trung bình 9 run).

**Nhận xét:**

UCI S0: tabular = 1 — CNN/BiLSTM tắt đúng thiết kế. Khi có G1/G2, mass dồn sang BiLSTM (~0.67–0.69). OULAD: tabular giảm 0.315 → 0.172 khi cutoff tăng; BiLSTM tăng 0.453 → 0.591. CNN ổn định ~0.23–0.25. Cổng **không** sụp về một nhánh trên OULAD, nhưng BiLSTM chiếm phần lớn khi chuỗi đủ dài. Đây không phải SHAP từng điểm.
...
Lệnh `python project.py db predict` đọc enrollment đã catalog, lấy `PredictionResult` đã materialize (`p`, `t`, `ŷ`, `H₂`), không train lại. Ngưỡng `t` là giá trị STOP đã khóa theo mốc. Đề tài **không** xây app/API giao diện; CLI chỉ để đọc kết quả mô hình đã khóa.

### 4.3.3. Chức năng Recommendation V

Lệnh `python project.py db recommend` chỉ nhận OULAD 20/35/50/75. Module đọc đúng `PredictionResult`, áp feasibility, năm EBM, rồi ghi `recommendation` + `recommendation_item`. 100% bị từ chối trước khi xếp hạng. Không ước lượng ATE.

### 4.3.4. Ví dụ một enrollment

Sinh viên `OULAD:631334`, CCC 2014B, mốc 20% — Hybrid `p = 0.2578`, `t = 0.18`, `ŷ = 1`, H₂ = 0.823 → Recommendation V **HUMAN_REVIEW**. Không train lại.

---

## 4.4. Phạm vi và hạn chế của thực nghiệm

Năm điểm **phải nói thẳng khi bảo vệ** (không phải gian lận, là giới hạn đã chọn):

1. Outer không mở → chưa có test cuối ngoài inner VALID.
2. 100% không phải early warning (mẫu còn lại dễ hơn; Rec V từ chối 100%).
3. Recommendation V không phải can thiệp nhân quả.
4. S0/20% không phải claim chính (thiếu chuỗi / mốc lạnh). Ưu thế Hybrid đặt ở S1/S2 và OULAD 35–75%.
5. Không có đường epoch trên checkpoint khóa — không vẽ loss giả.

Bổ sung: confusion/PR/ROC research join nhãn `final_result` vào OOF (file OOF gốc không chứa target). Không xây giao diện người dùng. Chi tiết: Chương 5.
