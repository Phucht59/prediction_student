...
def test_resolver_prefers_exact_release_authority(tmp_path):
    local = tmp_path / "local.pt"
    release = tmp_path / "release.pt"
    _write(local, b"local-authority")
    ...
    assert result["resolved_checkpoint_source"] == "release_lfs"
    assert result["checkpoint_path"] == str(release)


def test_resolver_falls_back_only_when_release_is_missing(tmp_path):
    local = tmp_path / "local.pt"
    release = tmp_path / "missing-release.pt"
    ...
    assert result["resolved_checkpoint_source"] == "historical_local_authority"
...
def test_resolver_fails_closed_on_present_local_sha_mismatch(tmp_path):
    local = tmp_path / "local.pt"
    release = tmp_path / "release.pt"
    _write(local, b"valid-local")
    _write(release, b"tampered-release")
