...
from scripts.release_freeze.build_freeze_manifest import build_manifest
...
from src.training.model_comparison import _selected_configs, make_model
from src.training.release_freeze import (
    _checkpoint_path,
    _run_id,
    status_payload,
    validate_freeze,
...
def test_final_runner_has_no_outer_selection_inputs() -> None:
    from src.training import release_freeze

    for function in (
        release_freeze._train_deep_final,
        release_freeze._fit_mlp_final,
        release_freeze._run_id,
    ):
        parameters = inspect.signature(function).parameters
        assert "outer_y_test" not in parameters
        assert "outer_labels" not in parameters
    source = inspect.getsource(release_freeze)
...
def test_status_schema_and_precommit_freeze_validation(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(release_freeze, "STATUS_PATH", tmp_path / "status.json")
    ...
    monkeypatch.setattr(release_freeze, "FREEZE_PATH", freeze_path)
