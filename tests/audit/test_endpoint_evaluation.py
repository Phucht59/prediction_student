...
from src.training.endpoint_evaluation import (
    ENDPOINT_ID,
    ENDPOINT_STAGE,
    FINAL_SEEDS,
    HISTORICAL_H0,
...
def test_outer_label_firewall_api() -> None:
    ...
    from src.training import endpoint_evaluation

    development_source = inspect.getsource(
        endpoint_evaluation.run_development_supervisor
    )
    assert "run_final_supervisor" not in development_source
    final_source = inspect.getsource(endpoint_evaluation.run_final_supervisor)
...
def test_final_integrity_firewall_flags_are_expected_false() -> None:
    ...
    source = inspect.getsource(endpoint_evaluation.run_final_supervisor)
    assert '"outer_labels_used_for_tuning",' in source
    assert '"post_outer_tuning",' in source
    assert "expected_false_checks" in source
    freeze_source = inspect.getsource(endpoint_evaluation._validate_freeze_commit)
