"""Configurable CNN + BiLSTM Hybrid with mask-safe adaptive fusion."""
...
from src.hybrid.models.components import BiLSTMBranch, ResidualCNNBranch, masked_mean_max
from src.hybrid.phase7.model import ResidualProjector


@dataclass(frozen=True)
class Phase8HybridConfig:
    static_dim: int
    temporal_dim: int
    aggregate_dim: int
    d_fuse: int = 96
...
class Phase8UnifiedHybrid(nn.Module):
    """A tabular-preserving parallel CNN/BiLSTM model.

    Progress reaches the gate directly; thus it changes predictions even when
    no optional interaction branch exists.  Unavailable temporal/aggregate
    branches receive -inf gate logits before softmax and therefore zero mass.
    """

    model_id = "hybrid"
    display_name = "Unified Hybrid"

    def __init__(self, config: Phase8HybridConfig):
        super().__init__()
        self.config = config
        d = config.d_fuse
        ...
        # Gate sees branch content, progress, and availability; its output is
        # evaluated per sample, rather than imposing hard-coded stage weights.
    ...
    def fusion_regularization(self) -> torch.Tensor:
        """Mild entropy floor only for F3; zero for every other fusion."""
