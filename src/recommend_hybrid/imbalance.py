...
measurement respectively. The study preserves the canonical frozen prediction
checkpoint and produces reporting evidence only.
"""
...
def select_validation_threshold(target: np.ndarray, probability: np.ndarray) -> float:
    """Select the F1-optimal threshold using validation rows only."""
    ...
    for threshold in candidates:
        ...
        key = (
            ...
            float(balanced_accuracy_score(y, prediction)),
...
def _metric_payload(
    ...
    p = np.asarray(probability, dtype=np.float64).reshape(-1)
    if len(y) != len(p) or not np.isfinite(p).all():
        raise ValueError("test probabilities must be finite and aligned")
    ...
    pr_auc = float(average_precision_score(y, p)) if np.any(y == 1) else float("nan")
    return {
        "roc_auc": roc_auc,
        "pr_auc": pr_auc,
...
def run_frozen_embedding_imbalance_study(
    *,
    ...
    """Run all locked modes with one identical linear prediction head."""
    ...
    for mode in IMBALANCE_MODES:
        ...
        row["warning"] = warning
        row["resampling_scope"] = "TRAIN_EMBEDDINGS_ONLY"
        row["canonical_checkpoint_replaced"] = False
        results.append(row)
    ...
    return {
        ...
        "canonical_prediction_checkpoint_replaced": False,
        "results": results,
    }
