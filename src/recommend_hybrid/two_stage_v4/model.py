"""Action-aware integrated heads over the frozen residual CNN-BiLSTM state.

V4 corrects the V3 false-issue failure without adding an external model.  The
candidate action head is supervised on every valid candidate, including the
all-zero action targets of non-recommendable groups.  A differentiable noisy-OR
provides an action-derived recommendability estimate and a consistency loss
aligns it with the direct group head.
"""
...
class HybridActionAwareRecommendationHeads(nn.Module):
    ...
    model_id = "hybrid_integrated_two_stage_v4_action_aware"
