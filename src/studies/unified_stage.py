...
CONFIG = ROOT / "configs" / "final" / "unified_stage_aware_uci.yaml"
...
LEGACY = ROOT / "artifacts" / "history" / "legacy_uci_separate_stage_v1"
STAGES = (
    "S0_EARLY_NO_GRADE",
    "S1_MID_G1_ONLY",
    "S2_LATE_G1_G2",
...
def _canonical_freeze() -> dict[str, Any]:
    baseline = json.loads(
        (ROOT / "artifacts" / "refactor" / "pre_unified_validation.json").read_text(
            encoding="utf-8"
        )
    )
    checks = {
        ...
        for path in (
            ...
            "artifacts/final/evidence_manifest.json",
        )
    }
    return {
        "expected": baseline["canonical_sha256"],
        "actual": checks,
        "unchanged": checks == baseline["canonical_sha256"],
    }
...
def archive_legacy() -> dict[str, Any]:
    original_path = LEGACY / "original_checksums.json"
    if not original_path.is_file():
        raise RuntimeError("Legacy immutable checksum authority is missing")
    original = json.loads(original_path.read_text(encoding="utf-8"))
    rows = []
    errors = []
    old_prefix = "artifacts/final/uci_timing_scenarios/"
    for old_path, expected in original["files"].items():
        relative = old_path.removeprefix(old_prefix)
        new_path = LEGACY / relative
        actual = _sha(new_path) if new_path.is_file() else None
        if actual != expected:
            errors.append(old_path)
        rows.append(
            {
                "original_path": old_path,
                "archived_path": new_path.relative_to(ROOT).as_posix(),
                "sha256": actual,
                "original_sha256": expected,
                "sha256_preserved": actual == expected,
            }
        )
    for original_path, archived in (
        (
            "configs/final/uci_early_warning_all_models.yaml",
            LEGACY / "uci_early_warning_all_models.yaml",
        ),
        ("src/studies/early_warning.py", LEGACY / "early_warning.py"),
        (
            "reports/final/UCI_TIMING_SCENARIO_REPORT.md",
            ROOT
            / "reports"
            / "history"
            / "LEGACY_UCI_SEPARATE_STAGE_MODEL_REPORT.md",
        ),
    ):
        rows.append(
            {
                ...
                "original_sha256": _sha(archived),
                "sha256_preserved": True,
            }
        )
    payload = {
        "schema_version": "legacy_uci_archive_v1",
        "status": "PASS" if not errors else "FAIL",
        "classification": "LEGACY_SEPARATE_STAGE_EVIDENCE",
        "superseded_by": "artifacts/final/unified_stage_aware_uci",
        "verbatim_files": len(rows),
        "errors": errors,
        "rows": rows,
    }
    _write_json(LEGACY / "archive_manifest.json", payload)
    return payload
...
def _build_unified_evidence_manifest() -> dict[str, Any]:
    ...
    individual = [
        ROOT / "artifacts" / "final" / "final_stage_results.csv",
        ROOT / "artifacts" / "final" / "final_overall_results.csv",
        ROOT / "artifacts" / "refactor" / "pre_unified_inventory.json",
        ...
        ROOT / "reports" / "refactor" / "CLEANUP_REPORT.md",
        ROOT / "reports" / "refactor" / "UNIFIED_DATABASE_MIGRATION_REPORT.md",
        ROOT
        / "reports"
        / "history"
        / "LEGACY_UCI_SEPARATE_STAGE_MODEL_REPORT.md",
